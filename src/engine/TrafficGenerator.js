...
export class TrafficGenerator {
  constructor(options = {}) {
    ...
    this.simTime = 0.0; // Start at midnight
    this.simSpeed = 1.0;
    this.isPaused = false;
  ...
  /**
   * Set a live traffic parameter (for interactive sliders)
   * @param {'baseLoad'|'peakLoad'|'burstChance'|'burstMultiplier'} key
   * @param {number} value
   */
  setParam(key, value) {
    if (this.pattern && key in this.pattern) {
      // Create a mutable copy of the pattern on first override
      if (!this._patternOverride) {
        this._patternOverride = true;
  ...
  update(nodes, links, deltaMs) {
    ...
    const dayFactor =
      0.5 + 0.5 * Math.sin((this.simTime / 24) * Math.PI * 2 - Math.PI / 2);
    ...
    sourceNodes.forEach((sourceNode, sourceIndex) => {
      ...
      const sourceModifier = NODE_TYPE_MODIFIERS[sourceNode.type] || {
        loadMult: 1,
        latencyMult: 1,
      };
      ...
      const burstIntensity =
        microBurst > burstThreshold
          ? (microBurst - burstThreshold) /
            Math.max(this.pattern.burstChance, 0.01)
          : 0;
      ...
      sourceDemandRatio +=
        burstIntensity * (this.pattern.burstMultiplier - 1) * 0.2;
      ...
      this.events
        .filter(
          (event) =>
            event.nodeId === sourceNode.id && event.type === "load_override",
        )
        .forEach((event) => {
          if (Date.now() - event.timestamp < event.duration) {
            sourceDemandRatio = clamp(
              Number(event.targetLoad) || sourceDemandRatio,
              0,
              1.4,
            );
          }
      ...
      let destinationIndex = Math.floor(
        stableHash01(`${sourceNode.id}:${timeBucket}:${sourceIndex}`) *
      ...
      if (!destinationNode || destinationNode.id === sourceNode.id) {
        destinationNode =
          destinationNodes.find((node) => node.id !== sourceNode.id) || null;
      }
      ...
      path.nodeIds.forEach((nodeId) => {
        const demandFactor =
          nodeId === sourceNode.id || nodeId === destinationNode.id ? 0.6 : 1;
        nodeDemandMbps.set(
          nodeId,
          (nodeDemandMbps.get(nodeId) || 0) + flowDemandMbps * demandFactor,
        );
      });
      ...
      path.linkIds.forEach((linkId) => {
        linkDemandMbps.set(
          linkId,
          (linkDemandMbps.get(linkId) || 0) + flowDemandMbps,
        );
      });
    });
    ...
    nodes.forEach((node) => {
      ...
      const modifier = NODE_TYPE_MODIFIERS[node.type] || {
        loadMult: 1,
        latencyMult: 1,
      };
      ...
      this.events
        .filter(
          (event) => event.nodeId === node.id && event.type === "load_override",
        )
        .forEach((event) => {
          if (Date.now() - event.timestamp < event.duration) {
            baselineLoad = clamp(
              Number(event.targetLoad) || baselineLoad,
              0,
              1.4,
            );
          }
      ...
      const routedDemandRatio =
        (nodeDemandMbps.get(node.id) || 0) / interfaceSpeedMbps;
      const nodeTargetLoad = clamp(
        baselineLoad * 0.4 + routedDemandRatio * 0.9,
        0,
        1,
      );
      state.currentLoad =
        state.currentLoad +
        (nodeTargetLoad - state.currentLoad) * this.options.smoothing;
      ...
      node.latency = Number(
        (3 + loadRatio * 30 * modifier.latencyMult).toFixed(2),
      );
    ...
    links.forEach((link) => {
      ...
      const utilizationCap = clamp(
        (Number(link.utilizationCap) || 100) / 100,
        0.1,
        1,
      );
      ...
      const lossPercent = clamp(
        baseLossPercent + congestionPenalty * 25 + networkStress * 10,
        0,
        100,
      );
      ...
      const status =
        utilization > 1.2 || loss > 0.12
          ? "lossy"
          : utilization > 0.85 || loss > 0.04
            ? "degraded"
            : "up";
      const flow = clamp(
        1 - Math.max(0, utilization - 0.85) * 0.8 - loss,
        0,
        1,
      );
  ...
  getSourceNodes(nodes) {
    const typedSources = nodes.filter((node) =>
      SOURCE_NODE_TYPES.has(node.type),
    );
    if (typedSources.length > 0) return typedSources;

    const nonDestinationNodes = nodes.filter(
      (node) => !DEST_NODE_TYPES.has(node.type),
    );
  ...
  getDestinationNodes(nodes) {
    const typedDestinations = nodes.filter((node) =>
      DEST_NODE_TYPES.has(node.type),
    );
    if (typedDestinations.length > 0) return typedDestinations;

    const infraNodes = nodes.filter(
      (node) =>
        node.type === "coreRouter" ||
        node.type === "router" ||
        node.type === "switch" ||
        node.type === "firewall",
    );
  ...
  findShortestPath(sourceId, targetId, adjacency, nodeById) {
    ...
    while (unvisited.size > 0) {
      ...
      if (currentNode === null || bestDistance === Number.POSITIVE_INFINITY)
        break;
